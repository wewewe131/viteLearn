<template>
    <div class="joinMain">
        ...
        <div v-else class="joinBox">
            ...
            <div class="inputBox">
                <input type="text" v-model="groupId" placeholder="请输入群聊ID" />
            </div>
            <div class="inputBox">
                <input type="text" v-model="mark" placeholder="备注(选填)" />
            </div>
            <div class="buttonBox">
                <MyButton size="2" radius="10px" @click="joinGroup()">加入</MyButton>
            </div>
        </div>
...
let msg = ""
function joinGroup() {
    group.joinGroup(groupId.value, mark.value).then(res => {
        console.log(res, mark.value, groupId.value);
...
let { groupId, showSuccess, mark } = toRefs(state)
